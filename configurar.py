...
def cargarConfig(values):
    ...
    if values['cantad'] == '0' and values['cantverb'] == '0' and values['cantsus'] == '0':
        values['cantad'] = 1
        values['cantverb'] = 1
        values['cantsus'] = 1


    if values['colorSus'] == values['colorVerb'] or values['colorSus'] == values['colorAd'] or values['colorAd'] == values['colorVerb']:

        sg.Popup('Los colores coinciden elija nuevos')
    else:
        ok = True
...
def config_main():
    ...
    layout = [
        ...
        [sg.Text('Orientacion del juego'),sg.InputCombo(['Vertical', 'Horizontal'], size=(40,20),key = 'orientacion', readonly=True)],
        [sg.Text('Letras en '),sg.InputCombo(['mayuscula', 'minuscula'] , size=(40,20),key = 'mayus', readonly=True)],
        [sg.Text('Palabras Agregadas', justification='center'), sg.Listbox(values= list(['  Sustantivos: '] +list(d['sustantivo'].keys()) + ['  Adjetivos: '] + list(d['adjetivo'].keys()) + ['  Verbos: '] +list(d['verbo'].keys())
                                                                                     ), key= 'listbox', size=(30,5))],
        [sg.Text('Cantidad de Palabras', justification='center', font=('Helvetica', 11))],
        [sg.Text('Sustantivos:', justification='center'), sg.InputCombo(list(range(len(list(d['sustantivo'].keys()))+1)), key= 'cantsus', size=(5,5), readonly=True),sg.Button(key ='colorSus',button_text='color',button_type=sg.BUTTON_TYPE_COLOR_CHOOSER)],
        [sg.Text('Adjetivos:    ', justification='center'), sg.InputCombo(list(range(len(list(d['adjetivo'].keys()))+1)), key= 'cantad', size=(5,5), readonly=True),sg.Button(key ='colorAd',button_text='color',button_type=sg.BUTTON_TYPE_COLOR_CHOOSER)],
        [sg.Text('Verbo:         ', justification='center'), sg.InputCombo(list(range(len(list(d['verbo'].keys())) + 1)), key= 'cantverb', size=(5,5), readonly=True),sg.Button(key ='colorVerb',button_text='color',button_type=sg.BUTTON_TYPE_COLOR_CHOOSER)],
        [sg.Text('Ayuda', justification='center'), sg.InputCombo(['Si','No'], key= 'ayuda', size=(5,5), readonly=True)],
        ...
        if event == 'Guardar configuracion':
            ...
            if p.isalpha() and not(ok) and p != '':
                ...
                if tipo != '9999':
                    ...
                    listbox.Update(['   Sustantivos: '] +list(d['sustantivo'].keys()) + ['  Adjetivos: '] + list(d['adjetivo'].keys()) + ['  Verbos: '] +list(d['verbo'].keys()))
